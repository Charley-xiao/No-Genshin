...
为了使得音乐更加丰富，我们实现了升降调的功能，具体来讲，当用户按下`butscale`对应的按钮时，自动模式下的播放会转换调式，目前可用的调式有C大调、F大调与B大调。在`AutoController`即自动模式下，`scale`用于保存当前的升降调状态，而具体的升降调对应关系则存储在`Buzzer`中。
